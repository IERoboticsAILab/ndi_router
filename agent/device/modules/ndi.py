...
class NDIModule(Module):
    ...
    def on_agent_connect(self) -> None:
        ...
        self.log.info("agent_connect: exported env NDI_PATH=%s extra=%s", ndi_path, list(ndi_env.keys()) if isinstance(ndi_env, dict) else None)
    ...
    def handle_cmd(self, action: str, params: Dict[str, Any]) -> tuple[bool, str | None, dict]:
        self.log.info("handle_cmd: action=%s params=%s", action, {k: ("***" if k in {"password", "token"} else v) for k, v in (params or {}).items()})
        ...
        if action == "set_input":
            ...
            if not src:
                ...
                return False, "missing source", {}
            self.log.info("set_input: requested source=%s restart=%s", src, bool(self.cfg.get("set_input_restart", True)))
